...
void CoOpRelayPanel::Draw()
{
	...
	DrawButton(Rectangle(Point(115., buttonY), Point(100., 30.)), "Address", canJoin, 'j');
	...
	font.Draw("LAN Discovery", body.TopLeft() + Point(24., 306.), bright);
	if(discovered.empty())
		font.Draw("No LAN relays found. Tailscale/WAN: press J and enter IP:port.",
			body.TopLeft() + Point(24., 332.), inactive);
	else
	{
		...
		for(int i = 0; i < discoveryLines; ++i)
		{
			...
			const Rectangle row = Rectangle::FromCorner(body.TopLeft() + Point(20., 328. + 20. * i), Point(720., 20.));
			if(canJoin)
			{
			...
			font.Draw(FitText(font, text, 700.), body.TopLeft() + Point(24., 332. + 20. * i),
	...
	font.Draw("Players", body.TopLeft() + Point(24., 408.), bright);
	int line = 0;
	if(CoOpRelayController::Get().IsConnected() && player.IsLoaded())
	{
		const string system = player.GetSystem() ? player.GetSystem()->TrueName() : "";
		font.Draw(FitText(font, LocalStatus(player), 700.), body.TopLeft() + Point(24., 434.), bright);
		++line;
		font.Draw(FitText(font, AuthorityStatus(player, authorityStore, remoteStore,
			...
			body.TopLeft() + Point(24., 434. + 20. * line), medium);
		++line;
		font.Draw(FitText(font, "Shared NPCs: " + to_string(localNPCs.size()), 700.),
			body.TopLeft() + Point(24., 434. + 20. * line), localNPCs.empty() ? inactive : medium);
		++line;
		font.Draw(FitText(font, MissionStateText(missionStates), 700.),
			body.TopLeft() + Point(24., 434. + 20. * line), missionStates.empty() ? inactive : medium);
		++line;
	}
	for(const CoOpRelay::RemotePresence &presence : remotes)
	{
		...
		const Rectangle row = Rectangle::FromCorner(body.TopLeft() + Point(20., 430. + 20. * line), Point(720., 20.));
		if(canSendResources)
		{
		...
		font.Draw(FitText(font, text, 700.), body.TopLeft() + Point(24., 434. + 20. * line),
			isTarget ? bright : medium);
		++line;
	}
	if(!line)
		font.Draw("Not connected to a relay.", body.TopLeft() + Point(24., 434.), inactive);
	else if(remotes.empty())
		font.Draw("Waiting for remote players.", body.TopLeft() + Point(24., 434. + 20. * line), inactive);
...
bool CoOpRelayPanel::KeyDown(SDL_Keycode key, Uint16, const Command &, bool)
{
	...
	{
		if(player.IsLoaded())
			GetUI().Push(DialogPanel::RequestString(
				this, &CoOpRelayPanel::JoinAddress, "Relay address:", "127.0.0.1:5050"));
		return true;
	}
	if(key == 'l')
	{
...
void CoOpRelayPanel::JoinAddress(const string &address)
{
	...
	CoOpRelayController::Get().Connect(std::move(endpoint.host), endpoint.port, PlayerName(player), roomPassword);
	UI::PlaySound(UI::UISound::NORMAL);
	EnterGame();
}
