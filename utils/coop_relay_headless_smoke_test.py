...
def snapshot(player_id: str, sequence: int, name: str, system: str = "Sol") -> str:
    return (
        f"snapshot\t3\t{sequence}\t{player_id}\t{name}\t{system}\t"
        "10\t-20\t0.5\t1.5\t45\t\tShuttle\t123\t0.9\t0.8\t0.7\t0.6\t0.5\t1"
    )


def require_snapshot_line(line: str, player_id: str, sequence: int, name: str, system: str) -> None:
    fields = line.split("\t")
...
async def run_smoke(exe: Path, repo: Path, timeout: float) -> None:
    if not exe.exists():
    ...
    try:
        ...
        require_authority_line(authority, "Sol", "player-1")
        ...
        writer_b.write((snapshot("player-2", 2, "Player B", "Alpha Centauri") + "\n").encode("utf-8"))
        await writer_b.drain()
        await asyncio.sleep(0.2)
        ...
        require_snapshot_line(cached_b, "player-2", 2, "Player B", "Alpha Centauri")
...
def main() -> int:
    ...
    try:
        asyncio.run(run_smoke(args.exe.resolve(), repo, args.timeout))
